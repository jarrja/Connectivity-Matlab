function [hE,hV]=eeg_wgPlot(adjMat,coord,varargin)
...
%     line([legLim(kk),legLim(kk+1)],[legY,legY],'lineWidth',15,'color',edgeMap(kk,:));
  ...
  % Draw Legend Label
  text([legLim(1),legLim(end)],[legY-1.8*yStep,legY-1.8*yStep], ...
    {num2str(0.00),num2str(1.00)},'HorizontalAlignment','center','color',[0,0,0]);
else
    nSegment=length(ii);
