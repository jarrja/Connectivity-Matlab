...
function EEG2 = firfilt(EEG, b, nFrames)
...
for iDc = 1:(length(dcArray) - 1)
        ...
        for iBlock = 1:(length(blockArray) - 1)
            ...
            % Filter the data
            %Edit by Jarr 10.23.15 EEG2, to prevent changing original data
            [EEG2.data(:, (blockArray(iBlock) - groupDelay):(blockArray(iBlock + 1) - groupDelay - 1)), zi] = ...
        ...
        %Edit by Jarr 10.23.15 EEG2, to prevent changing original data
        EEG2.data(:, (dcArray(iDc + 1) - ziDataDur):(dcArray(iDc + 1) - 1)) = ...
...
if EEG.trials > 1
    %Edit by Jarr 10.23.15 EEG2, to prevent changing original data
    EEG2.data = reshape(EEG.data, [EEG.nbchan EEG.pnts EEG.trials]);
end
